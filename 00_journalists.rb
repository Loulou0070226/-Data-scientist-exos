...
tab.each {i=i+1}
puts " Il y a #{i} handle dans cette array"

        
tab.each { |m|  #M = string que je suis en train de regarder 
    if m.length < str.length 
        str = m
    end
}
puts "le handle le plus court de cette liste est #{str}"
...
puts "Il y a #{i} handles qui contiennent 5 caractères"
...
tab = tab.sort_by {|q| q.length}
tab.each  {|q| 
    puts q
}

tab.each_with_index {|r,s| 
    if r == "@epenser"
        puts "La personne @epenser a la position #{s} dans l'array"
    end
}
